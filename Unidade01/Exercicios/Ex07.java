...
public class Ex07 {
    public static void main(String[] args) {
        ...
        while (jogoAtivo) {
            ...
            if (testeVitoria(tabuleiro, jogadorAtual)) {
                imprimirTabuleiro(tabuleiro);
                System.out.println("Jogador " + jogadorAtual + " venceu!");
                jogoAtivo = false;
            } else if (testeEmpate(tabuleiro)) {
                imprimirTabuleiro(tabuleiro);
    ...
    public static boolean testeVitoria(char[][] tabuleiro, char jogador) {
        for (int i = 0; i < 3; i++) {
    ...
    public static boolean testeEmpate(char[][] tabuleiro) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (tabuleiro[i][j] == ' ') {
                    return false;
